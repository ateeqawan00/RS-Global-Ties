...
export const sendOTPEmailHelp = async (name, email,query,footerMessage) => {
    return new Promise(async (resolve, reject) => {
        try {
            const transporter = nodemailer.createTransport({
                service: 'gmail',
                auth: {
                    user: process.env.NewEMAIL, // your email
                    pass: process.env.APP_PASSWORD // your app password
                }
            });

            const mailOptions = {
                from: `B2B  <${process.env.NewEMAIL}>`,
                to: email,
                name: `Sender name ${name}`,
                query: `<h1>sender query ${query}</h1>
                ${footerMessage ? `<p>${footerMessage}</p>` : ''}`
