...
export default function CategoriesByCountry() {
  ...
  useEffect(() => {
    if (country) {
      ...
      const filtered = products.filter(
        (product) => product.user?.country?.toLowerCase() === lowerCaseCountry
      )
  ...
  // console.log("fil p:", filteredProducts);
      ...
      <TabsBody className='flex flex-col'>
        {filteredProducts.length === 0 ? (
          <div className='text-center mt-4'>
            No products found from {country}.
          </div>
        ) : (
          filteredProducts
            ?.filter(
