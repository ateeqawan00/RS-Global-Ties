...
export default function CategoriesByPlatinumMember() {
    ...
    const fetchProducts = async () => {
      try {
        const { data } = await axiosInstance.post(
          "/product/getProductsBySubscription",
          { planKey }
        )
        console.log("fetch products log: ", data)
        setProducts(data)
      } catch (error) {
  ...
  return (
    <Tabs value={activeTab} className='p-4 min-h-screen'>
      ...
      >
        ...
        <div className='dropdown flex-1 '>
          <div className='rounded-md py-2 px-4  bg-[#ecf0f9] text-center text-nowrap text-primary font-semibold'>
            <select
            ...
            .map((item, index) => (
              <TabPanel key={index} value={activeTab} className='p-4 gap-8'>
                <Link to={`/product/${item._id}`}>
                  <div
                    key={index}
                    className='flex justify-between w-full bg-white shadow-md rounded-md border p-4'
                  >
                    {/* left side content */}
                  ...
                  {item.user ? (
                    ...
                    <div>
                      <div className='flex items-center gap-4'>
                        ...
                        <div>
                          {item.user && item.user.isCompanyVerified === true ? (
